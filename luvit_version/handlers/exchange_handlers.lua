...
local function handleExchangeClothComplete(ctx)
    ...
    ctx.sendResponse(ResponseBuilder.build(2901, ctx.userId, 0, writer:toString()))
...
local function handleExchangePetComplete(ctx)
    local writer = BinaryWriter.new()
    writer:writeUInt32BE(0)
    ctx.sendResponse(ResponseBuilder.build(2902, ctx.userId, 0, writer:toString()))
...
local function handleExchangeOre(ctx)
    ...
    ctx.sendResponse(ResponseBuilder.build(2251, ctx.userId, 0, writer:toString()))
...
local function handleExchangeNewYear(ctx)
    local writer = BinaryWriter.new()
    writer:writeUInt32BE(0)
    ctx.sendResponse(ResponseBuilder.build(2065, ctx.userId, 0, writer:toString()))
...
local function handleTalkCount(ctx)
    ...
    writer:writeUInt32BE(0)      -- ret
    writer:writeUInt32BE(0)      -- count
    ctx.sendResponse(ResponseBuilder.build(2701, ctx.userId, 0, writer:toString()))
...
local function handleTalkCate(ctx)
    ...
    ctx.sendResponse(ResponseBuilder.build(2702, ctx.userId, 0, writer:toString()))
