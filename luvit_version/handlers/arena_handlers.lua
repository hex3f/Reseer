...
local function handleChoiceFightLevel(ctx)
    ...
    ctx.sendResponse(ResponseBuilder.build(2414, ctx.userId, 0, writer:toString()))
...
local function handleStartFightLevel(ctx)
    local writer = BinaryWriter.new()
    writer:writeUInt32BE(0)
    ctx.sendResponse(ResponseBuilder.build(2415, ctx.userId, 0, writer:toString()))
...
local function handleLeaveFightLevel(ctx)
    ctx.sendResponse(ResponseBuilder.build(2416, ctx.userId, 0, ""))
...
local function handleArenaSetOwner(ctx)
    local writer = BinaryWriter.new()
    writer:writeUInt32BE(0)
    ctx.sendResponse(ResponseBuilder.build(2417, ctx.userId, 0, writer:toString()))
...
local function handleArenaFightOwner(ctx)
    local writer = BinaryWriter.new()
    writer:writeUInt32BE(0)
    ctx.sendResponse(ResponseBuilder.build(2418, ctx.userId, 0, writer:toString()))
...
local function handleArenaGetInfo(ctx)
    local writer = BinaryWriter.new()
    writer:writeUInt32BE(0)                     -- flag (0=无人占领)
    writer:writeUInt32BE(0)                     -- hostID
    writer:writeStringFixed("", 16)             -- hostNick
    writer:writeUInt32BE(0)                     -- hostWins
    writer:writeUInt32BE(0)                     -- challengerID
    ctx.sendResponse(ResponseBuilder.build(2419, ctx.userId, 0, writer:toString()))
...
local function handleArenaUpfight(ctx)
    local writer = BinaryWriter.new()
    writer:writeUInt32BE(0)
    ctx.sendResponse(ResponseBuilder.build(2420, ctx.userId, 0, writer:toString()))
...
local function handleFightSpecialPet(ctx)
    local writer = BinaryWriter.new()
    writer:writeUInt32BE(0)
    ctx.sendResponse(ResponseBuilder.build(2421, ctx.userId, 0, writer:toString()))
...
local function handleArenaOwnerAcce(ctx)
    local writer = BinaryWriter.new()
    writer:writeUInt32BE(0)
    ctx.sendResponse(ResponseBuilder.build(2422, ctx.userId, 0, writer:toString()))
...
local function handleArenaOwnerOut(ctx)
    local writer = BinaryWriter.new()
    writer:writeUInt32BE(0)
    ctx.sendResponse(ResponseBuilder.build(2423, ctx.userId, 0, writer:toString()))
...
local function handleOpenDarkportal(ctx)
    local writer = BinaryWriter.new()
    writer:writeUInt32BE(0)
    ctx.sendResponse(ResponseBuilder.build(2424, ctx.userId, 0, writer:toString()))
...
local function handleFightDarkportal(ctx)
    local writer = BinaryWriter.new()
    writer:writeUInt32BE(0)
    ctx.sendResponse(ResponseBuilder.build(2425, ctx.userId, 0, writer:toString()))
...
local function handleLeaveDarkportal(ctx)
    ctx.sendResponse(ResponseBuilder.build(2426, ctx.userId, 0, ""))
...
local function handleFreshChoiceFightLevel(ctx)
    local writer = BinaryWriter.new()
    writer:writeUInt32BE(0)
    ctx.sendResponse(ResponseBuilder.build(2428, ctx.userId, 0, writer:toString()))
...
local function handleFreshStartFightLevel(ctx)
    local writer = BinaryWriter.new()
    writer:writeUInt32BE(0)
    ctx.sendResponse(ResponseBuilder.build(2429, ctx.userId, 0, writer:toString()))
...
local function handleFreshLeaveFightLevel(ctx)
    ctx.sendResponse(ResponseBuilder.build(2430, ctx.userId, 0, ""))
